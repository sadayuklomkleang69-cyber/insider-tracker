...
# รหัสผ่านที่จาร์วิสส่องมาให้ท่านประธาน
...
try:
    ...
    if menu == "🐳 ระบบจับตาปลาวาฬ":
        ...
        with col1:
            ...
            for _, row in buys_df.sort_values('Date', ascending=False).head(10).iterrows():
                st.info(f"**{row['Symbol']}** | {row['Date'].strftime('%d/%m/%y')} \n\n {int(row['Shares']):,} หุ้น @ ${row.get('Price', 0):.2f}")
        with col2:
    ...
    # --- 5. PAGE: CALCULATOR ---
        ...
        with c1:
            ...
            final_avg = ((dime_shares * dime_avg) + top_up) / (dime_shares + new_sh)
    ...
    # --- 6. PAGE: LINE SYSTEM ---
        ...
        test_msg = st.text_input("พิมพ์ข้อความทดสอบ:", "จาร์วิสรายงานตัวครับท่านประธาน!")
        if st.button("🚀 ส่งข้อความทดสอบเข้า LINE"):
            ...
            if res and res.status_code == 200:
                ...
                st.error("ส่งไม่สำเร็จ ตรวจสอบ Token อีกครั้งครับ")
